function DESPAWNING_FAR_RANGE(owner)
  local groupmember = 1
  ...
  repeat
    if IsDead(owner) == false then
      ...
      repeat
        if IsDead(GetGroupMember(Unit_Group.WFUnits, groupmember)) == false and IsInRange(GetGroupMember(Unit_Group.WFUnits, groupmember), owner, 480) == true then
          PlayerInRange = true
          break
        end
