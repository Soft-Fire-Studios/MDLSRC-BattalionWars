function OWNER_DESPAWNER(owner)
  local groupmember = 1
  ...
  repeat
    if IsDead(owner) == false then
      ...
      repeat
        if IsDead(GetGroupMember(Unit_Group.AIarmy, groupmember)) == false and IsInRange(GetGroupMember(Unit_Group.AIarmy, groupmember), owner, 350) == true then
          PlayerInRange = true
          break
        end
