function OWNER_DESPAWNER_BUILDINGS(owner)
  local groupmember = 1
  ...
  repeat
    if IsDead(owner) == false then
      ...
      repeat
        if IsDead(GetGroupMember(Unit_Group.AIarmy, groupmember)) == false and IsInRange(GetGroupMember(Unit_Group.AIarmy, groupmember), owner, 480) == true then
          PlayerInRange = true
          break
        end
