function TroopDespawner(owner)
  ...
  repeat
    if IsDead(owner) == false then
      ...
      repeat
        if IsDead(GetGroupMember(Unit_Group.WFUnits, groupmember)) == false and IsInRange(GetGroupMember(Unit_Group.WFUnits, groupmember), owner, 300) == true then
          PlayerInRange = true
          break
        end
