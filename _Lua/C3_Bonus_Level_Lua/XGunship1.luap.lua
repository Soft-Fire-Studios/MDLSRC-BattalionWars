function XGunship1(owner)
  repeat
    EndFrame()
  until LevelState >= 4
  local GotTarget = 0
  repeat
    if IsDead(Air_Vehicle.XGunship1) and NumTimesGunshipsDead < TotalGunshipsToKill then
      if LevelState >= 4 then
        SetObjectiveData(Objective_Marker.XGunship1Marker, constant.OBJECTIVE_MARKER_DATA_VISIBLE, 0)
      end
      if LevelState >= 4 then
      ...
      repeat
        ...
        WaitFor(1)
      until IsReadyToRevive(Air_Vehicle.XGunship1)
      WaitFor(2)
      if IsInArea(GetPlayerUnit(), -763, -463, 650) then
        Teleport(Air_Vehicle.XGunship1, -567, 670, 135)
      else
        Teleport(Air_Vehicle.XGunship1, -763, -463, 45)
      end
      EndFrame()
    ...
    if IsDead(Ground_Vehicle.SHeavyTank1) == false and GotTarget == 0 then
      AttackTarget(Air_Vehicle.XGunship1, Ground_Vehicle.SHeavyTank1)
      GotTarget = 1
    end
    if IsDead(Ground_Vehicle.SHeavyTank2) == false and GotTarget == 0 then
      AttackTarget(Air_Vehicle.XGunship1, Ground_Vehicle.SHeavyTank2)
      GotTarget = 2
    end
    if IsDead(Ground_Vehicle.SHeavyTank3) == false and GotTarget == 0 then
      AttackTarget(Air_Vehicle.XGunship1, Ground_Vehicle.SHeavyTank3)
      GotTarget = 3
    end
    if IsDead(Ground_Vehicle.SHeavyTank4) == false and GotTarget == 0 then
      AttackTarget(Air_Vehicle.XGunship1, Ground_Vehicle.SHeavyTank4)
      GotTarget = 4
    end
    if IsDead(Ground_Vehicle.SHeavyTank5) == false and GotTarget == 0 then
      AttackTarget(Air_Vehicle.XGunship1, Ground_Vehicle.SHeavyTank5)
      GotTarget = 5
    end
    if IsDead(Ground_Vehicle.SHeavyTank6) == false and GotTarget == 0 then
      AttackTarget(Air_Vehicle.XGunship1, Ground_Vehicle.SHeavyTank6)
      GotTarget = 6
    end
    if GotTarget > 0 then
    ...
    EndFrame()
  until IsDead(Air_Vehicle.XGunship1) and NumTimesGunshipsDead == TotalGunshipsToKill
