function XBomber5(owner)
  repeat
    EndFrame()
  until LevelState >= 4
  local GotTarget = 0
  repeat
    if IsDead(Air_Vehicle.XBomber5) and NumTimesBombersDead < TotalBombersToKill then
      if LevelState >= 4 then
        SetObjectiveData(Objective_Marker.XBomber5Marker, constant.OBJECTIVE_MARKER_DATA_VISIBLE, 0)
      end
      if LevelState >= 4 then
      ...
      repeat
        if IsReadyToRevive(Air_Vehicle.XBomber5) then
          DebugOut("Aircraft Ready")
        end
        WaitFor(1)
      until IsReadyToRevive(Air_Vehicle.XBomber5)
      WaitFor(2)
      if IsInArea(GetPlayerUnit(), 425, -875, 650) then
        Teleport(Air_Vehicle.XBomber5, -1000, -200, 90)
      else
        Teleport(Air_Vehicle.XBomber5, 425, -875, 315)
      end
      EndFrame()
      ReviveDeadUnit(Air_Vehicle.XBomber5)
      DebugOut("Respawned Aircraft")
      EndFrame()
      SpawnUnitInCarrier(flag.TYPE_XYLVANIAN, flag.GTYPE_GRUNT, Air_Vehicle.XBomber5)
      GotTarget = 0
      if LevelState >= 4 then
    ...
    if IsDead(Ground_Vehicle.SHeavyTank1) == false and GotTarget == 0 then
      AttackTarget(Air_Vehicle.XBomber5, Ground_Vehicle.SHeavyTank1)
      GotTarget = 1
    end
    if IsDead(Ground_Vehicle.SHeavyTank2) == false and GotTarget == 0 then
      AttackTarget(Air_Vehicle.XBomber5, Ground_Vehicle.SHeavyTank2)
      GotTarget = 2
    end
    if IsDead(Ground_Vehicle.SHeavyTank3) == false and GotTarget == 0 then
      AttackTarget(Air_Vehicle.XBomber5, Ground_Vehicle.SHeavyTank3)
      GotTarget = 3
    end
    if IsDead(Ground_Vehicle.SHeavyTank4) == false and GotTarget == 0 then
      AttackTarget(Air_Vehicle.XBomber5, Ground_Vehicle.SHeavyTank4)
      GotTarget = 4
    end
    if IsDead(Ground_Vehicle.SHeavyTank5) == false and GotTarget == 0 then
      AttackTarget(Air_Vehicle.XBomber5, Ground_Vehicle.SHeavyTank5)
      GotTarget = 5
    end
    if IsDead(Ground_Vehicle.SHeavyTank6) == false and GotTarget == 0 then
      AttackTarget(Air_Vehicle.XBomber5, Ground_Vehicle.SHeavyTank6)
      GotTarget = 6
    end
    if GotTarget > 0 then
    ...
    EndFrame()
  until IsDead(Air_Vehicle.XBomber5) and NumTimesBombersDead == TotalBombersToKill
  Kill(Air_Vehicle.XBomber5)
  SetObjectiveData(Objective_Marker.XBomber5Marker, constant.OBJECTIVE_MARKER_DATA_VISIBLE, 0)
end
