function XBomber1(owner)
  repeat
    EndFrame()
  until LevelState >= 2
  local GotTarget = 0
  repeat
    if IsDead(Air_Vehicle.XBomber1) and NumTimesBombersDead < TotalBombersToKill then
      if LevelState >= 4 then
        SetObjectiveData(Objective_Marker.XBomber1Marker, constant.OBJECTIVE_MARKER_DATA_VISIBLE, 0)
      end
      if LevelState >= 4 then
      ...
      WaitFor(2)
      if IsInArea(GetPlayerUnit(), 470, -595, 650) then
        Teleport(Air_Vehicle.XBomber1, -500, -600, 90)
      else
        Teleport(Air_Vehicle.XBomber1, 470, -595, 315)
      end
      EndFrame()
    ...
    if IsDead(Ground_Vehicle.SHeavyTank1) == false and GotTarget == 0 then
      AttackTarget(Air_Vehicle.XBomber1, Ground_Vehicle.SHeavyTank1)
      GotTarget = 1
    end
    if IsDead(Ground_Vehicle.SHeavyTank2) == false and GotTarget == 0 then
      AttackTarget(Air_Vehicle.XBomber1, Ground_Vehicle.SHeavyTank2)
      GotTarget = 2
    end
    if IsDead(Ground_Vehicle.SHeavyTank3) == false and GotTarget == 0 then
      AttackTarget(Air_Vehicle.XBomber1, Ground_Vehicle.SHeavyTank3)
      GotTarget = 3
    end
    if IsDead(Ground_Vehicle.SHeavyTank4) == false and GotTarget == 0 then
      AttackTarget(Air_Vehicle.XBomber1, Ground_Vehicle.SHeavyTank4)
      GotTarget = 4
    end
    if IsDead(Ground_Vehicle.SHeavyTank5) == false and GotTarget == 0 then
      AttackTarget(Air_Vehicle.XBomber1, Ground_Vehicle.SHeavyTank5)
      GotTarget = 5
    end
    if IsDead(Ground_Vehicle.SHeavyTank6) == false and GotTarget == 0 then
      AttackTarget(Air_Vehicle.XBomber1, Ground_Vehicle.SHeavyTank6)
      GotTarget = 6
    end
    if GotTarget > 0 then
    ...
    EndFrame()
  until IsDead(Air_Vehicle.XBomber1) and NumTimesBombersDead == TotalBombersToKill
  Kill(Air_Vehicle.XBomber1)
  SetObjectiveData(Objective_Marker.XBomber1Marker, constant.OBJECTIVE_MARKER_DATA_VISIBLE, 0)
end
